...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        tries to a
        dd more Ants than is allowed, an assertion error is raised.
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        stay = self.place
        while stay != hive:
            if stay.bees is hive.bees:
                break
            if stay.bees and self.min_range <= enter <= self.max_range:
                return random_or_none(stay.bees)
            else:
                stay = stay.entrance
                enter += 1
        return random_or_none(stay.bees) and None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting:
            ...
            swallow = random_or_none(self.place.bees)
            if swallow:
                self.eat_bee(swallow)
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        if other.is_container == False and self.is_container and self.contained_ant is None:
            return True
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.real is False:
            ...
            while tracker is not None:
                if tracker.ant is not None:
                    if tracker.ant not in self.double:
                        tracker.ant.damage = tracker.ant.damage * 2
                        self.double.append(tracker.ant)
                    if tracker.ant.is_container == True and tracker.ant.contained_ant is not None and tracker.ant.contained_ant not in self.double:
                        tracker.ant.contained_ant.damage = tracker.ant.contained_ant.damage * 2
                        self.double.append(tracker.ant.contained_ant)
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    return effect(bee.action)(duration)
    # END Problem EC
